...
tuple<vector<int>, vector<int>, vector<int>> parse(string filename) {
    ...
    return make_tuple(vx, pos_y, pos_z);
}

void update(vector<int> &pos, vector<int> &vel, int n = 1) {
    for (int i = 0; i < n; i++) {
        int idx = 0;
        for (auto &v : vel) {
            for (auto x : pos) {
...
int part_1(vector<int> pos_x, vector<int> pos_y, vector<int> pos_z) {
    int k = pos_x.size();
    vector<int> vel_x(k, 0), vel_y(k, 0), vel_z(k, 0);
    ...
    int res = 0;
    for (int i = 0; i < k; i++) {
        pot = abs(pos_x[i]) + abs(pos_y[i]) + abs(pos_z[i]);
        kin = abs(vel_x[i]) + abs(vel_y[i]) + abs(vel_z[i]);
        res += pot * kin;
    }
...
int find_cycle(vector<int> &pos) {
    int k = pos.size();
    vector<int> vel(k, 0), zero(k, 0), pos_copy = pos;
    long iter = 0;
...
int main(int argc, char *argv[]) {
    ...
    tie(pos_x, pos_y, pos_z) = parse(filename);
